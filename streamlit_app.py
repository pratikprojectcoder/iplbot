...
MATCHES_CLEAN_PATH = "processed/matches_clean.csv"
IPL_JSON_DIR = "ipl_json"

# =========================================================
# NEW : MATCH EXPLAINER DATASET PATHS
# =========================================================

# CSV index containing all IPL matches
MATCH_INDEX_PATH = "processed/match_index.csv"

# Folder containing raw JSON match files
...
def load_match_index():
    try:
        df = pd.read_csv(MATCH_INDEX_PATH)

        # Clean columns
        df.columns = [c.strip() for c in df.columns]

        # Convert to string safely
        for col in ["team1", "team2", "venue", "date"]:
        ...
        st.error(f"Could not load match_index.csv: {e}")
        return pd.DataFrame()
...
def load_selected_match_json(json_path):
    try:
        full_path = os.path.join(MATCHES_FOLDER, json_path)
        ...
        st.error(f"Could not load match JSON: {e}")
        return None
...
def match_selector_ui():

    match_index_df = load_match_index()

    if match_index_df.empty:
    ...
    seasons = sorted(
        match_index_df["season"].dropna().unique().tolist(),
        reverse=True
    )
    ...
    all_teams = sorted(
        list(
            set(
                match_index_df["team1"].dropna().tolist() +
                match_index_df["team2"].dropna().tolist()
            )
        )
    )
    ...
    # ---------------------------------------------
    # Filter by season + team
    # ---------------------------------------------

    filtered_df = match_index_df[
        (
            (match_index_df["team1"] == team) |
            (match_index_df["team2"] == team)
        ) &
        (match_index_df["season"] == season)
    ].copy()
    ...
    filtered_df = filtered_df[
        (
            ((filtered_df["team1"] == team) &
             (filtered_df["team2"] == opponent))
            |
            ((filtered_df["team2"] == team) &
             (filtered_df["team1"] == opponent))
        )
    ].copy()

    if filtered_df.empty:
        st.warning("No matches found against selected opponent.")
        return None
    ...
    selected_row = filtered_df[
        filtered_df["match_label"] == selected_match
    ].iloc[0]
    ...
    raw_data = load_selected_match_json(
        selected_row["json_path"]
    )
...
def load_match_index():
    ...
    for filename in os.listdir(IPL_JSON_DIR):
        ...
        file_path = os.path.join(IPL_JSON_DIR, filename)
        try:
        ...
        match_date = pd.to_datetime(dates[0], errors="coerce")
    ...
    match_df = match_df.dropna(subset=["match_id", "date", "team_a", "team_b", "venue"]).copy()
    match_df = match_df.sort_values(["date", "match_id"], ascending=[False, False]).reset_index(drop=True)
    return match_df


def load_match_data_by_id(match_id):
    try:
        json_path = os.path.join(IPL_JSON_DIR, f"{int(match_id)}.json")
    except Exception:
        return None, "Invalid match id selected."

    if not os.path.exists(json_path):
        return None, f"Match JSON file not found: {json_path}"

    try:
        with open(json_path, "r", encoding="utf-8") as fp:
            raw_data = json.load(fp)
        return normalize_match_data(raw_data), None
    except Exception as e:
        return None, f"Could not parse selected match JSON: {e}"


def load_raw_match_json(match_id):
    try:
        json_path = os.path.join(IPL_JSON_DIR, f"{int(match_id)}.json")
    except Exception:
        return None
...
def format_wicket_kind(wk, bowler_name):
    ...
    fielder_names = [fielder_name(f) for f in fielders if f]

    if kind == "bowled":
        return f"b {bowler_name}"
    if kind == "caught":
        fn = fielder_names[0] if fielder_names else "?"
        return f"c {fn} b {bowler_name}"
    ...
    if kind == "run out":
        fn = fielder_names[0] if fielder_names else ""
        return f"run out ({fn})" if fn else "run out"
    if kind == "stumped":
        fn = fielder_names[0] if fielder_names else "?"
        return f"st {fn} b {bowler_name}"
    if kind == "hit wicket":
        return f"hit wkt b {bowler_name}"
...
def innings_to_scorecard(innings_obj):
    team = innings_obj.get("team", "Unknown")
    batting_order = []
    batting = {}

    def ensure_batter(name):
    ...
    for over_obj in innings_obj.get("overs", []):
        for ball in over_obj.get("deliveries", []):
            ...
            bwl = ensure_bowler(bowler)
            bwl["runs"] += rt
            if "wides" not in extras:
                bwl["balls"] += 1
                legal_balls_innings += 1

            wkts = ball.get("wickets", [])
            if wkts:
                bwl["wickets"] += len(wkts)
                for wk in wkts:
                    po = wk.get("player_out")
                    if po:
                        ensure_batter(po)
                        if batting[po]["dismissal"] is None:
                            batting[po]["dismissal"] = format_wicket_kind(wk, bowler)

            total_runs += rt

    dismissal_count = sum(1 for n in batting_order if batting[n]["dismissal"] is not None)

    bat_rows = []
    for name in batting_order:
        stt = batting[name]
        dismiss = stt["dismissal"] if stt["dismissal"] is not None else "not out"
        sr = round(100 * stt["runs"] / stt["balls"], 2) if stt["balls"] > 0 else 0.0
        bat_rows.append({
            "Batter": name,
            "R": stt["runs"],
            "B": stt["balls"],
    ...
    bowl_rows = []
    for name, stt in bowling.items():
        legal_balls = stt["balls"]
        overs_float = legal_balls // 6 + (legal_balls % 6) / 10.0
        econ = round(stt["runs"] / overs_float, 2) if overs_float > 0 else 0.0
        bowl_rows.append({
            "Bowler": name,
            "O": round(overs_float, 1),
            "R": stt["runs"],
            "W": stt["wickets"],
            "Econ": econ
        })
    ...
    if not bowl_df.empty:
        bowl_df = bowl_df.sort_values("W", ascending=False).reset_index(drop=True)

    overs_label = f"{legal_balls_innings // 6}.{legal_balls_innings % 6}"

    return {
        "team": team,
        "total_runs": total_runs,
        "total_wickets": dismissal_count,
        "overs_bowled": overs_label,
        "batting": bat_df,
        "bowling": bowl_df
    }


def build_match_scorecards(raw_data):
    if not raw_data or "innings" not in raw_data:
        return []

    scorecards = []
    for inn in raw_data.get("innings", []):
        if not inn.get("overs"):
            continue
        scorecards.append(innings_to_scorecard(inn))
    return scorecards
...
def generate_advanced_insights(match_data):
    ...
    a_runs = team_a_score["total_runs"]
    b_runs = team_b_score["total_runs"]
    b_wkts = team_b_score["total_wickets"]

    # team_a = first innings; team_b = second (chasing). Defended wins → margin in runs;
    # successful chase → margin in wickets remaining (10 − wickets lost in chase).
    if a_runs == b_runs:
        winner = "Tie"
        margin = 0
        margin_type = "runs"
        ...
        winner = team_a_name
        margin = a_runs - b_runs
        margin_type = "runs"
    else:
        winner = team_b_name
        margin = max(0, 10 - b_wkts)
        margin_type = "wickets"
...
def generate_fallback_summary(insights, match_data):
    ...
    margin_type = insights.get("margin_type", "runs")
    win_prob = insights["win_probability"]
    team_a_name = match_data.get("team_a", "Team A")

    if winner == "Tie":
        summary = "The match ended in a tie."
    elif margin_type == "wickets":
        wk_label = "wicket" if margin == 1 else "wickets"
        summary = f"{winner} won the match by {margin} {wk_label}."
    else:
        summary = f"{winner} won the match by {margin} runs."
...
with st.sidebar:
    with st.expander("React UI & FastAPI (separate from Streamlit)", expanded=False):
        st.markdown(
            """
This page is **only** the Streamlit app (`app.py`). It does **not** embed the React
frontend or the FastAPI server — those are started with other commands.

**React dashboard + REST API**

1. In a terminal at the project root, run:
   `python -m uvicorn api.main:app --reload --host 127.0.0.1 --port 8000`
2. Build the web app once: `cd web` then `npm run build`  
   *(or for live editing:* `npm run dev` *and open the Vite URL, usually port 5173.)*
3. Open **[http://127.0.0.1:8000](http://127.0.0.1:8000)** after a build, or use Vite’s URL in dev mode.
...
if app_mode == "Scorecard Explainer":
    st.markdown("#### Select Match Inputs")
    match_index_df = load_match_index()

    if match_index_df is None or match_index_df.empty:
        st.error("Could not build match index from ipl_json/. Add JSON match files there.")
        st.stop()

    seasons = sorted(match_index_df["season"].dropna().unique().tolist(), reverse=True)
    selected_season = st.selectbox("Season", seasons, key="se_season")

    season_df = match_index_df[match_index_df["season"] == selected_season].copy()

    season_teams = sorted(set(season_df["team_a"].tolist()) | set(season_df["team_b"].tolist()))
    selected_team = st.selectbox("Team", season_teams, key="se_team")

    team_rows = season_df[(season_df["team_a"] == selected_team) | (season_df["team_b"] == selected_team)].copy()
    opponent_options = sorted({
        row["team_b"] if row["team_a"] == selected_team else row["team_a"]
        for _, row in team_rows.iterrows()
    })

    if not opponent_options:
        st.warning("No opponents found for selected team in this season.")
        st.stop()

    selected_opponent = st.selectbox("Opponent", opponent_options, key="se_opponent")

    matchup_rows = season_df[
        ((season_df["team_a"] == selected_team) & (season_df["team_b"] == selected_opponent)) |
        ((season_df["team_a"] == selected_opponent) & (season_df["team_b"] == selected_team))
    ].copy()

    venue_options = sorted(matchup_rows["venue"].dropna().unique().tolist())
    if not venue_options:
        st.warning("No venues found for this team-opponent pair in selected season.")
        st.stop()

    selected_venue = st.selectbox("Venue", venue_options, key="se_venue")

    filtered_matches = matchup_rows[matchup_rows["venue"] == selected_venue].copy()
    filtered_matches = filtered_matches.sort_values("date", ascending=False)

    if filtered_matches.empty:
        st.warning("No match found for the selected filters.")
        st.stop()

    match_records = filtered_matches.to_dict("records")
    selected_match = st.selectbox(
        "Match",
        match_records,
        format_func=lambda m: (
            f"{pd.to_datetime(m['date']).date()} | {m['team_a']} vs {m['team_b']} "
            f"| {m['venue']} | Match ID {int(m['match_id'])}"
        ),
        key="se_match_id"
    )

    raw_scorecard = load_raw_match_json(selected_match["match_id"])

    st.markdown("### Scorecard")
    if raw_scorecard:
        with st.expander("Batting & bowling by innings", expanded=True):
            info_block = raw_scorecard.get("info", {})
            outcome = info_block.get("outcome", {})
            toss = info_block.get("toss", {})
            if toss:
                st.caption(
                    f"Toss: {toss.get('winner', '')} chose to {toss.get('decision', '')}."
                )
            if outcome:
                win = outcome.get("winner")
                by = outcome.get("by", {}) or {}
                if win and by:
                    kind = next(iter(by.keys()), "")
                    margin = by.get(kind, "")
                    st.caption(f"Result: **{win}** won by **{margin}** {kind}.")
                elif win:
                    st.caption(f"Result: **{win}**.")

            scorecards = build_match_scorecards(raw_scorecard)
            if not scorecards:
                st.info("No innings with deliveries found in this file.")
            else:
                for idx, sc in enumerate(scorecards, start=1):
                    st.markdown(
                        f"**Innings {idx} — {sc['team']}**  "
                        f"*{sc['total_runs']}/{sc['total_wickets']} ({sc['overs_bowled']} ov)*"
                    )
                    st.markdown("Batting")
                    st.dataframe(sc["batting"], hide_index=True, use_container_width=True)
                    st.markdown("Bowling")
                    st.dataframe(sc["bowling"], hide_index=True, use_container_width=True)
    else:
        st.warning("Could not load this match file for a scorecard.")

    st.divider()

    match_data, load_error = load_match_data_by_id(selected_match["match_id"])
    if load_error:
        st.error(load_error)
        st.info("If a scorecard appears above, that data still came from the same match file.")
        st.stop()

    is_valid, issues = validate_match_data(match_data)
    if not is_valid:
        st.error("Automated match analysis needs two innings in the JSON (powerplay / momentum charts).")
        st.write("Details:", issues)
        st.info("The scorecard above still reflects whatever innings exist in the file.")
        st.stop()
